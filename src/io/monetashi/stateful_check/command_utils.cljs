...
(defn generate-command-name
  ...
  (if-let [generate-command (:model/generate-command spec)]
    ...
    (if-let [valid-commands (->> (keys (:commands spec))
      ...
      (throw (js/Error "All commands failed `:model/requires` check: cannot generate a valid command!")))))
...
(defn run-command
  ...
  (if-let [real-command (:real/command command)]
    ...
    (throw (js/Error (str "No :real/command function found for " (:name command) " command")))))
